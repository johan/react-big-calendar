...
if ('geolocation' in navigator) {
  let narrowToClosestSite = ({coords}) => {
    ...
    console.log({site});
    haievents = haievents.filter((event) => event.in === site);
    window.selectable = selectable;
    selectable && selectable.render();
  ...
  narrowToClosestSite({ coords: { latitude: 37.4, longitude: -122.1 }});
  navigator.geolocation.getCurrentPosition(
    narrowToClosestSite,
    () => { console.error('failed to geolocate visitor');
  });
}
